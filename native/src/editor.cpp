...
void Editor::draw_stage(Rectangle area) {
    ...
    constexpr float VOLUME_WIDTH = 140.0f;   // the card, padding included
...
void Editor::draw_volume_panel(Rectangle area) {
    ...
    // A card of its own, level with the middle of the booth beside it. Loose on
    // the panel the three sliders read as stray controls; boxed, they read as
    // one set - and the web editor draws the same box.
    constexpr float GROUP = 48.0f;
    constexpr float HEAD = 26.0f;
    constexpr float PAD = 10.0f;

    const float card_height = HEAD + GROUP * 3.0f - 13.0f + PAD * 2.0f;
    const Rectangle card = {area.x, area.y + std::max(0.0f, (area.height - card_height) / 2.0f),
                            area.width, card_height};
    gui::panel(card, gui::UI_CARD);
    DrawRectangleLinesEx(card, 1.0f, gui::UI_EDGE);

    const float left = card.x + PAD;
    const float width = card.width - PAD * 2.0f;
    float y = card.y + PAD;

    gui::label({left, y}, "VOLUME", 14.0f, gui::UI_AMBER);
    y += HEAD;
    ...
    for (const Row& row : rows) {
        ...
        gui::label({left, y}, row.name, 13.0f, gui::UI_INK_DIM);
        gui::label({left + width - gui::text_width(percent, 13.0f), y}, percent, 13.0f,
                   gui::UI_INK_DIM);
        if (gui::slider({left, y + 15.0f, width, 20.0f}, *row.value)) moved = true;
        y += GROUP;
    }
