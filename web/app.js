...
/**
 * The booth's first lift of the visit - and the rattle is the point of it.
 *
 * A browser only lets a page make noise once the visitor has touched it, so
 * opening on a timer meant the first rise of the shutter was always silent.
 * A muted probe finds out whether sound is allowed yet: if it is (a return
 * visit, usually), the booth opens on its own; if not, it stays shut and the
 * first click or key anywhere lifts it, rattle and all - the booth opening
 * for you when you show up, which is how the game does it anyway.
 */
async function firstOpen() {
  ...
  // Ask by doing: try to play the very sound the lift makes. Allowed - a
  // return visit, usually - the rattle is under way and the booth opens with
  // it. Refused, nothing has made a noise and the booth waits to be touched.
  try {
  ...
  const onFirstTouch = (event) => {
    ...
    // A first click on the lever itself is left to the lever: its own handler
    // is about to open the booth, and doing it here as well would close it.
    if (event.target && event.target.id === 'lever') return;
    if ($('booth').classList.contains('closed')) openShutter();
  };
